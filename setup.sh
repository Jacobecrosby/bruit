...
# Install BRUIT in editable mode with correct extras
if $IS_MAC; then
    echo "🍎 Installing BRUIT with macOS dependencies"
    pip install -e '.[mac]'
elif $IS_WINDOWS || $IS_LINUX; then
    echo "🖥️ Installing BRUIT with GPU dependencies"
    pip install -e '.[gpu]'
fi

echo "✅ BRUIT setup complete!"
